...
const ProjectSection: React.FC<ProjectSectionProps> = ({ title, description, imageSrc, link, reverse }) => (
  <div className={`flex flex-col md:flex-row w-full mt-20 ${reverse ? 'md:flex-row-reverse' : ''}`}>
    ...
    <div className="md:w-1/2 text-right md:text-left flex flex-col justify-center">
      ...
      <ul className="list-none flex flex-row gap-2 pt-3 md:text-sm text-xs font-medium">
        <li className="rounded-2xl md:h-[40px] h-[30px] md:w-[120px] w-[90px] flex items-center justify-center bg-gradient-to-r from-[#f4b6c2] via-[#f4b6c2] to-pink-400 text-white">
          HTML
        </li>
        <li className="rounded-2xl md:h-[40px] h-[30px] md:w-[120px] w-[90px] flex items-center justify-center bg-gradient-to-r from-[#f4b6c2] via-[#f4b6c2] to-pink-400 text-white">
          TailwindCSS
